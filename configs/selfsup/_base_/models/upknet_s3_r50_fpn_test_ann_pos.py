...
model = dict(
    ...
    rpn_head=dict(
        ...
        loss_mask_pos=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),
        loss_dice_pos=dict(type='DiceLoss', loss_weight=4.0)),
    roi_head=dict(
        type='KernelIterHead',
    ...
    train_cfg=dict(
        rpn=dict(
            assigner=dict(
                type='MaskHungarianPositionAssigner',
                cls_cost=dict(type='FocalLossCost', weight=2.0),
                dice_cost=dict(type='DiceCost', weight=4.0, pred_act=True),
                mask_cost=dict(type='MaskCost', weight=1.0, pred_act=True),
                dice_cost_pos=dict(type='DiceCost', weight=4.0, pred_act=True),
                mask_cost_pos=dict(type='MaskCost', weight=1.0, pred_act=True)),
