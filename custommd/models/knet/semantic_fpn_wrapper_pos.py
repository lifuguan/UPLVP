...
class SemanticFPNWrapper(nn.Module):
    ...
    def forward(self, inputs): # 256 * 1; 256 * 1/2; 256 * 1/4; 256 * 1/8; 
        mlvl_feats = []
        # 不同尺度缩放到同一大小
        for i in range(self.start_level, self.end_level + 1):
            input_p = inputs[i]
            if i == self.cat_coors_level:
                if self.positional_encoding is not None:
                    ...
                    input_p = torch.cat([input_p, positional_encoding], dim=0)
                if self.cat_coors:
        ...
        if self.num_aux_convs > 0:
            ...
            for conv in self.aux_convs:
                outs.append(conv(feature_add_all_level))  # pixel-wise分割，用于获取蒙版
            return outs
