...
class KernelIterHead(BaseRoIHead):
    ...
    def _mask_forward(self, stage, x, object_feats, mask_preds, img_metas, if_eval=False, text_features=None):
        mask_head = self.mask_head[stage] # kernel_update_head.py (forward func)
        cls_score_head, cls_score_clip, mask_preds, object_feats = mask_head(
            x, object_feats, mask_preds, if_eval=if_eval, text_features=text_features, img_metas=img_metas)
        ...
        mask_results = dict(
            cls_score_head=cls_score_head,
            cls_score_clip=cls_score_clip,
    ...
    def forward_train(self,
        ...
        if cls_score_clip is not None:
            prev_cls_score_clip = cls_score_clip.detach()
        else:
            prev_cls_score_clip = [None] * num_imgs
        ...
        # num_stages代表有多少个KernelIterHead，在此处进行迭代

        for stage in range(self.num_stages):
            mask_results = self._mask_forward(stage, x, object_feats,
                                              mask_preds, img_metas, if_eval=if_eval, text_features=text_features)
            all_stage_mask_results.append(mask_results)
            mask_preds = mask_results['mask_preds']
            scaled_mask_preds = mask_results['scaled_mask_preds']
            cls_score = mask_results['cls_score_head']
            cls_score_clip = mask_results['cls_score_clip']
            ...
            for i in range(num_imgs):
                if stage < self.assign_stages:  # 只在训练过程中使用匈牙利匹配算法
                    if img_metas[i]['ori_filename'] == '000000189095.jpg':
                        a = True
                    ...
                    if prev_cls_score[i] is not None:
                        ...
                        cls_for_assign = None
                    if prev_cls_score_clip[i] is not None:
                        cls_for_assign_clip = prev_cls_score_clip[
                            i][:self.num_proposals, :self.num_thing_classes]
                    else:
                        cls_for_assign_clip = None ################
                    assign_result = self.mask_assigner[stage].assign(
                        mask_for_assign, cls_for_assign, cls_for_assign_clip, gt_masks[i],
                ...
                # note: 断点调试的时候用于提取网络的特征图和预测得分
                # if img_metas[i]['ori_filename'] == '000000189095.jpg':
                #     from debugger import save_train_info
                #     save_train_info(i, img_metas, assign_result.gt_inds, cls_score, scaled_mask_preds, gt_masks)
                #     pass
            mask_targets = self.mask_head[stage].get_targets( 
                # return: labels, label_weights, mask_targets, mask_weights
                sampling_results,  
                gt_masks,    # 每个非空类别所对应的蒙版, unused
                gt_labels,   # 这个是除去背景的labels, unused
                self.train_cfg[stage],
                True,
                gt_sem_seg=gt_sem_seg,
                gt_sem_cls=gt_sem_cls)

            # 单阶段损失：kernel_update_head.py loss()
    ...
    def simple_test(self,
                    x, # feature map 
                    proposal_feats,  # kernel
                    mask_preds,  # predict masks
                    cls_score,  # score
        ...
        target = mask_preds # TEST
        object_feats = proposal_feats
        for stage in range(self.num_stages):
            mask_results = self._mask_forward(stage, x, object_feats,
                                              mask_preds, img_metas, if_eval=True)
        ...
        if self.do_panoptic:
            ...
            for img_id in range(num_imgs):
                ...
                target_per_img = target[img_id][mask_indices]
        ...
        # save_test_info(img_metas, scores_per_img, scaled_mask_preds)
        # save_train_info(0,None, None, None,scaled_mask_preds=scaled_mask_preds, gt_masks=None)
