...
class MaskHungarianClipAssigner(BaseAssigner):
    ...
    def __init__(self,
        ...
        self.cls_cost_clip = build_match_cost(cls_cost_clip)
    ...
    def assign(self,
        ...
        if self.cls_cost.weight != 0 and cls_pred is not None:
            ...
            cls_cost = 0
        if self.cls_cost_clip.weight != 0 and cls_pred_clip is not None:
            cls_cost_clip = self.cls_cost_clip(cls_pred_clip, gt_labels) # 该分类只针对类别分类，同类别不同实例损失值一致
        else:
            cls_cost_clip = 0
        ...
        if self.boundary_cost is not None and self.boundary_cost.weight != 0:
            ...
            b_cost = 0
        cost = cls_cost + cls_cost_clip + reg_cost + dice_cost + b_cost  # -1.6, -1.4
