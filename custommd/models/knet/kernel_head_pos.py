...
class ConvKernelHead(nn.Module):
    ...
    def __init__(self,
        ...
        if loss_mask_pos is not None:
            ...
            self.loss_mask_pos = loss_mask_pos
        ...
        if loss_dice_pos is not None:
            self.loss_dice_pos = build_loss(loss_dice_pos)
        else:
            self.loss_dice_pos = loss_dice_pos
    ...
    def _init_layers(self):
        ...
        self.init_kernels = nn.Conv2d(
            ...
            bias=False)
        # position kernel
        self.init_kernels_pos = nn.Conv2d(
            self.out_channels,
            self.num_proposals,
            self.conv_kernel_size,
            padding=int(self.conv_kernel_size // 2),
    ...
    def init_weights(self):
        ...
        if self.xavier_init_kernel:
            ...
            nn.init.xavier_uniform_(self.init_kernels_pos.weight)
        else:
            logger = get_root_logger()
            logger.info(
                f'Initialize kernels by normal std: {self.kernel_init_std}')
            normal_init(self.init_kernels, mean=0, std=self.kernel_init_std)
            normal_init(self.init_kernels_pos, mean=0, std=self.kernel_init_std)

    def _decode_init_proposals(self, img, img_metas):
        num_imgs = len(img_metas)
        
        # semantic FPN wrapper
        # localization_feats[0] : 特征图
        # localization_feats[1] : 粗蒙版
        ...
        for conv in self.loc_convs:
            loc_feats = conv(loc_feats)  # 1*1 conv
        if self.feat_downsample_stride > 1 and self.feat_refine:
            loc_feats = self.ins_downsample(loc_feats)
        mask_preds = self.init_kernels(loc_feats)  # 降维到100，对应mask和kernel
        pos_preds = self.init_kernels_pos(loc_feats)

        mask_preds = mask_preds * pos_preds

        # mask_preds可视化
        # from debugger import save_test_info, save_train_info
        # save_train_info(0,None, None, None,scaled_mask_preds=mask_preds, gt_masks=None)
        ...
        if self.semantic_fpn:
            ...
            for conv in self.seg_convs:  # 1*1 conv
        ...
        if semantic_feats is not None:
            seg_preds = self.conv_seg(semantic_feats)  # channel: 256 -> 1
        else:
            seg_preds = None

        proposal_feats = self.init_kernels.weight.clone()  # kernel采取卷积核的权重
        proposal_feats_pos = self.init_kernels_pos.weight.clone()
        proposal_feats = torch.cat([proposal_feats, proposal_feats_pos], dim=0) # proposal_feats shape [200,256,1,1]
        proposal_feats = proposal_feats[None].expand(num_imgs,
                                                     *proposal_feats.size()) # proposal_feats shape [bs,200,256,1,1]
        ...
        if self.proposal_feats_with_obj:   # instance kernel param + semantic feat 
            obj_feats = obj_feats.view(num_imgs, self.num_proposals, self.out_channels, 1, 1)
            obj_feats = torch.cat([obj_feats, obj_feats], dim=1)
            proposal_feats = proposal_feats + obj_feats
        ...
        return proposal_feats, x_feats, mask_preds, cls_scores, seg_preds, pos_preds
    ...
    def forward_train(self,
        ...
        (proposal_feats, x_feats, mask_preds, cls_scores, seg_preds, pos_preds) = results
        if self.feat_downsample_stride > 1:
            scaled_mask_preds = F.interpolate(
                ...
                mode='bilinear',
                align_corners=False)
            scaled_pos_preds = F.interpolate(
                pos_preds,
                scale_factor=self.feat_downsample_stride,
                mode='bilinear',
                align_corners=False)
        ...
        sampling_results, sampling_results_pos = [], []
        if cls_scores is None:
            detached_cls_scores = [None] * self.bs
        else:
            detached_cls_scores = cls_scores.detach()

        for i in range(num_imgs):
            with open('example.txt', 'w') as f:
                f.write(str(scaled_mask_preds[i].shape))
                f.write(str(scaled_pos_preds[i].shape))
                f.write(str(gt_masks[i].shape))
                f.write(str(gt_poses[i].shape))
            assign_result = self.assigner.assign(scaled_mask_preds[i].detach(),
                                                 scaled_pos_preds[i].detach(),
                                                 detached_cls_scores[i],
                                                 gt_masks[i], gt_poses[i], gt_labels[i],
                                                 img_metas[i])
            ...
            sampling_result_pos = self.sampler.sample(assign_result,
                                                  scaled_pos_preds[i],
                                                  gt_poses[i])
        ...
        losses = self.loss(scaled_mask_preds, cls_scores, scaled_seg_preds, scaled_pos_preds,
                           proposal_feats, pos_targets, *mask_targets)
        ...
        return losses, proposal_feats, x_feats, mask_preds, cls_scores, pos_preds
    ...
    def loss(self,
        ...
        if pos_inds.any():
            ...
            pos_pos_pred = pos_pred.reshape(num_preds, H, W)[bool_pos_inds]
            pos_pos_targets = pos_targets[2][bool_pos_inds]
            losses['loss_rpn_mask_pos'] = self.loss_mask_pos(pos_pos_pred,
                                                     pos_pos_targets)
            losses['loss_rpn_dice_pos'] = self.loss_dice_pos(pos_pos_pred,
                                                     pos_pos_targets)
            ...
            losses['loss_rpn_mask_pos'] = mask_pred.sum() * 0
            losses['loss_rpn_dice_pos'] = mask_pred.sum() * 0
    ...
    def get_targets(self,
        ...
        pos_mask_list = [res.pos_masks for res in sampling_results] #(2,200,234)
        neg_mask_list = [res.neg_masks for res in sampling_results] #(98,200,234)
