...
class KernelUpdateHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_mask_pos=dict(
                     type='CrossEntropyLoss', use_mask=True, loss_weight=1.0),
                 loss_dice_pos=dict(type='DiceLoss', loss_weight=3.0),
                 loss_cls=dict(
                     type='FocalLoss',
                     use_sigmoid=True,
                     gamma=2.0,
        ...
        self.loss_mask_pos = build_loss(loss_mask_pos)
        self.loss_dice_pos = build_loss(loss_dice_pos)
        ...
        self.mask_fcs_pos = nn.ModuleList()
        for _ in range(num_mask_fcs):
            self.mask_fcs_pos.append(
                nn.Linear(in_channels, in_channels, bias=False))
            self.mask_fcs_pos.append(
                build_norm_layer(dict(type='LN'), in_channels)[1])
            self.mask_fcs_pos.append(build_activation_layer(act_cfg))

        self.fc_mask_pos = nn.Linear(in_channels, out_channels)
    ...
    def forward(self,
        ...
        if self.feat_transform is not None:
            x = self.feat_transform(x) # 1*1 conv
        C, H, W = x.shape[-3:]
        ...
        # mask 和 image feature 作卷积，卷成kernel的shape
        x_feat = torch.einsum('bnhw,bchw->bnc', sigmoid_masks, x) # x_feat shape [bs,100,256]
        x_feat = torch.cat([x_feat, x_feat], dim=1) # x_feat shape [bs,200,256]

        # kernel reshape
        # obj_feat in shape [B, N, C, K, K] -> [B, N, C, K*K] -> [B, N, K*K, C]
        proposal_feat = proposal_feat.reshape(N, num_proposals,
                                              self.in_channels,
                                              -1).permute(0, 1, 3, 2) # shape [bs,200,1,256]
        # kernel_updator.py
        # x_feat -> refined mask; proposal_feat -> reshaped kernel
        # obj_feat in shape [N, C, K * K]
        obj_feat = self.kernel_update_conv(x_feat, proposal_feat)

        # [B, N, K*K, C] -> [B, N, K*K*C] -> [N, B, K*K*C]
        obj_feat = obj_feat.reshape(N, num_proposals, -1).permute(1, 0, 2) # shape [200,bs,256]
        ...
        obj_feat = obj_feat.reshape(N, num_proposals, -1, self.in_channels) # [bs,200,1,256]

        # Fead-Foward Netwoek + LN
        if self.with_ffn:
            obj_feat = self.ffn_norm(self.ffn(obj_feat)) # [bs,200,1,256]

        cls_feat = obj_feat.sum(-2)[:, :100, :]  # used to generate cls_score  # [bs,100,256]
        mask_feat = obj_feat[:, :100, :, :]  # used to generate mask  # [bs,100,1,256]
        pos_feat = obj_feat[:, 100:, :, :]  # [bs,100,1,256]

        for cls_layer in self.cls_fcs:
            cls_feat = cls_layer(cls_feat)
        for reg_layer in self.mask_fcs:
            mask_feat = reg_layer(mask_feat)
        for reg_layer_pos in self.mask_fcs_pos:
            pos_feat = reg_layer_pos(pos_feat)

        # [B, N, K * K] -> [B, N, C]
        cls_score = self.fc_cls(cls_feat).view(N, num_proposals // 2, -1)
        # [B, N, K*K, C] -> [B, N, C, K*K]
        mask_feat = self.fc_mask(mask_feat).permute(0, 1, 3, 2)
        pos_feat = self.fc_mask_pos(pos_feat).permute(0, 1, 3, 2)
        ...
        # [B, N, C, K*K] -> [B*N, C, K, K]
        mask_feat = mask_feat.reshape(N, num_proposals // 2, C,
                                      self.conv_kernel_size,
                                      self.conv_kernel_size) # shape [bs,100,256,1,1]
        pos_feat = pos_feat.reshape(N, num_proposals // 2, C,
                                      self.conv_kernel_size,
                                      self.conv_kernel_size) # shape [bs,100,256,1,1]
        
        # [B, C, H, W] -> [1, B*C, H, W]
        new_mask_preds, new_pos_preds = [], []
        for i in range(N):
        ...
        new_pos_preds = torch.cat(new_pos_preds, dim=0)

        new_mask_preds = new_mask_preds.reshape(N, num_proposals // 2, H, W)
        new_pos_preds = new_pos_preds.reshape(N, num_proposals // 2, H, W)

        new_mask_preds = new_mask_preds * new_pos_preds
        ...
        return cls_score, new_mask_preds, new_pos_preds, obj_feat.permute(0, 1, 3, 2).reshape(
    ...
    def loss(self,
        ...
        # 选择的蒙版位置
        pos_inds = (labels >= 0) & (labels < bg_class_ind)  # 求取labels为非空([0 : num_classes-1])的位置，空类别为bg_class_ind
        ...
        if mask_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_mask'] = self.loss_mask(pos_mask_pred,
                                                     pos_mask_targets)
                losses['loss_dice'] = self.loss_dice(pos_mask_pred,  # 选择的实例
                                                     pos_mask_targets) # 真值
                
                pos_pos_pred = pos_pred.reshape(num_preds, H,
                                                  W)[bool_pos_inds]
                pos_pos_targets = pos_targets[2][bool_pos_inds]
                losses['loss_mask_pos'] = self.loss_mask(pos_pos_pred,
                                                     pos_pos_targets)
                losses['loss_dice_pos'] = self.loss_dice(pos_pos_pred,  # 选择的实例
                                                     pos_pos_targets) # 真值
                ...
                losses['loss_mask_pos'] = mask_pred.sum() * 0
                losses['loss_dice_pos'] = mask_pred.sum() * 0
    ...
    def _get_target_single(self, pos_inds, neg_inds, pos_mask, neg_mask,
        ...
        num_pos = pos_mask.size(0) # positvie mask 的数量
        num_neg = neg_mask.size(0) # negative mask 的数量
        num_samples = num_pos + num_neg
        H, W = pos_mask.shape[-2:]
        # original implementation uses new_zeros since BG are set to be 0
        # now use empty & fill because BG cat_id = num_classes,
        # FG cat_id = [0, num_classes-1]
        labels = pos_mask.new_full((num_samples, ),  # 全体赋值background值
                                   self.num_classes,
                                   dtype=torch.long)
        ...
        if num_pos > 0:
            labels[pos_inds] = pos_gt_labels  # gt位置赋值对应类别
            pos_weight = 1.0 if cfg.pos_weight <= 0 else cfg.pos_weight # positive 权重
    ...
    def get_targets(self,
        ...
        pos_inds_list = [res.pos_inds for res in sampling_results]  # assign为正样本的list
        neg_inds_list = [res.neg_inds for res in sampling_results]  # assign为负样本的list
        ...
        if gt_sem_seg is None:
            gt_sem_seg = [None] * self.bs
            gt_sem_cls = [None] * self.bs
    ...
    def segm2result(self, mask_preds, det_labels, cls_scores):
        ...
        # real bboxes
        bboxes = []
        for i, mask_pred in enumerate(mask_preds):
            bbox = mask2bbox(torch.tensor(mask_pred[None, ...]))
            bbox = np.hstack((bbox.detach().numpy()[0], cls_scores[i]))
            bboxes.append(bbox)
