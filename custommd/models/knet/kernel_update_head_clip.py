...
class KernelUpdateHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_cls_clip = build_loss(loss_cls_clip)
        ...
        self.cls_clip_fcs = nn.ModuleList()
        assert num_cls_clip_fcs == 1
        for _ in range(num_cls_clip_fcs):
            self.cls_clip_fcs.append(
                nn.Linear(in_channels, clip_channels, bias=False))
            self.cls_clip_fcs.append(
                build_norm_layer(dict(type='LN'), clip_channels)[1])
            self.cls_clip_fcs.append(build_activation_layer(act_cfg))
    ...
    def forward(self,
        ...
        cls_feat = obj_feat.sum(-2)  # used to generate cls_score [B, N, C]
        mask_feat = obj_feat  # used to generate mask [B, N, K*K, C]
        cls_feat_head = cls_feat

        # cls_feat shape [B, N, C]

        for cls_layer in self.cls_fcs:
            cls_feat_head = cls_layer(cls_feat_head)
        for reg_layer in self.mask_fcs:
            mask_feat = reg_layer(mask_feat)

        # [B, N, K * K] -> [B, N, C]
        cls_score_head = self.fc_cls(cls_feat_head).view(N, num_proposals, -1)

        # clip class score
        cls_score_clip = None
        if not if_eval: # only when training we use clip for classification
            for cls_clip_layer in self.cls_clip_fcs:
                cls_feat = cls_clip_layer(cls_feat)

            cls_score_clip = cls_feat @ text_features
        ...
        return cls_score_head, cls_score_clip, new_mask_preds, obj_feat.permute(0, 1, 3, 2).reshape(
            N, num_proposals, self.in_channels, self.conv_kernel_size,
            self.conv_kernel_size)

    @force_fp32(apply_to=('cls_score', 'cls_score_clip', 'mask_pred'))
    def loss(self,
             object_feats,  # kernel
        ...
        if cls_score_clip is not None:
            if cls_score_clip.numel() > 0:
                losses['loss_cls_clip'] = self.loss_cls_clip( # Focal loss
                    cls_score_clip.view(num_preds, -1),
                    labels,
                    label_weights,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override)
                # cls_score中的类别分数最大的为最可能的类别
                losses['pos_acc_clip'] = accuracy(
                    cls_score_clip.view(num_preds, -1)[pos_inds], labels[pos_inds])
