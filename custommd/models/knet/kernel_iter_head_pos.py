...
class KernelIterHead(BaseRoIHead):
    ...
    def _mask_forward(self, stage, x, object_feats, mask_preds, img_metas):
        ...
        cls_score, mask_preds, pos_preds, object_feats = mask_head(
        ...
        mask_results = dict(
            ...
            pos_preds=pos_preds,
            scaled_pos_preds=scaled_pos_preds,
    ...
    def forward_train(self,
                      x,
                      proposal_feats, # shape [bs,200,256,1,1]
                      mask_preds,
                      pos_preds,
                      cls_score,
                      img_metas,
        ...
        if self.mask_head[0].mask_upsample_stride > 1:
            ...
            prev_pos_preds = pos_preds.detach()
        ...
        object_feats = proposal_feats # [bs,200,256,1,1]
        all_stage_loss = {}
        ...
        for stage in range(self.num_stages):
            ...
            pos_preds = mask_results['pos_preds']
            scaled_pos_preds = mask_results['scaled_pos_preds']
            cls_score = mask_results['cls_score']
            object_feats = mask_results['object_feats']

            if self.post_assign:
                prev_mask_preds = scaled_mask_preds.detach()
                prev_pos_preds = scaled_pos_preds.detach()
                prev_cls_score = cls_score.detach()
            ...
            for i in range(num_imgs):
                if stage < self.assign_stages:  # 只在训练过程中使用匈牙利匹配算法
                    ...
                    pos_for_assign = prev_pos_preds[i][:self.num_proposals]
                    ...
                    assign_result = self.mask_assigner[stage].assign(
                        mask_for_assign, pos_for_assign, cls_for_assign, gt_masks[i], gt_poses[i],
                ...
                sampling_result_pos = self.mask_sampler[stage].sample(
                    assign_results[i], scaled_pos_preds[i], gt_poses[i])
            ...
            pos_targets = self.mask_head[stage].get_targets( 
                # return: labels, label_weights, mask_targets, mask_weights
