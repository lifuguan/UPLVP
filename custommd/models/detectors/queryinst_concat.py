...
class QueryInstConcat(QueryInst):
    def __init__(self,
                 backbone,
                 rpn_head,
                 roi_head,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(QueryInstConcat, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def forward_train(self,
        ...
        if self.is_kernel2query is True:
            patch_feats = patch_feats.view(bs, 20, -1).contiguous()
            proposal_feats = torch.cat([proposal_feats,patch_feats], dim=1)

            # proposal_feats = proposal_feats + patch_feats.permute(1,0,2).contiguous()
    ...
    def extract_patches(self, imgs, gt_masks, img_metas, feats):
        ...
        for i, (per_img_mask, img) in enumerate(zip(gt_masks, imgs)):
            if len(per_img_mask) == 0: # filter images which don't have gt_masks
                print(img_metas[i])
