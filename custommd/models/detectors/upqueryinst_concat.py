...
class UpQueryInstConcat(QueryInst):
    def __init__(self,
                 backbone,
                 rpn_head,
                 roi_head,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(UpQueryInstConcat, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def forward_train(self,
        ...
        if self.is_kernel2query is True:
            ...
            proposal_feats = torch.cat([proposal_feats, refine_patch_feats[idx, ...] \
                .permute(1,0,2).contiguous()], dim=2)
            proposal_feats = self.linear(proposal_feats)
    ...
    def extract_patches(self, imgs, gt_masks, img_metas, feats):
        ...
        for i, (per_img_mask, img) in enumerate(zip(gt_masks, imgs)):
            ...
            for box in boxes:
                ...
                pusedo_kernels.append(self.avgpool(patch_feat).flatten(1).permute(1, 0))
