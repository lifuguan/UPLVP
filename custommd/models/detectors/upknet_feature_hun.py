...
class UPKNet(KNet):
    ...
    def forward_train(self,
        ...
        patch_feats, expend_labels = self.extract_patches(img, gt_masks, gt_labels, img_metas, x[-1]) ######
        ...
        if self.is_kernel2query is True:
            patch_feats = patch_feats.view(bs, 20, -1) \
                ...
                .permute(1, 0, 2) \
                .contiguous()
            expend_labels = expend_labels.view(bs, 20, -1) \
                .repeat_interleave(self.num_proposals // 20, dim=1) \
                .contiguous() ######
            _expend_labels = [] ######

            for per_img_labels in expend_labels: ######
                per_img_expend_labels = F.one_hot(per_img_labels.T, num_classes=80)
                _expend_labels.append(per_img_expend_labels)
            expend_labels = torch.cat(_expend_labels, dim=0) ###### shape=[bs,num_kernels,num_classes]
    ...
    def extract_patches(self, imgs, gt_masks, gt_labels, img_metas, feats): ######
        """Directly extract features from the backbone+neck."""

        ins_per_img, pusedo_kernels, kernel_labels = [], [], [] ######


        for i, (per_img_mask, per_img_label, img) in enumerate(zip(gt_masks, gt_labels, imgs)): ######
            if len(per_img_mask) == 0:
                print(img_metas[i])
                patch_feat_refine = torch.zeros((1,256), device=img.device)
                pusedo_kernels.append(patch_feat_refine)
                label = torch.zeros([1,1], device=img.device) ######
                ...
                for _, (box, label) in enumerate(zip(boxes, per_img_label)): ######
                    if box[1] == box[3]:
                        box[3] += 2
                    if box[0] == box[2]:
                        box[2] += 2
        ...
        kernel_labels = torch.cat(kernel_labels, dim=0) ######


        begin_i, expend_kernels, expend_labels = 0, [], []
        for end_i in ins_per_img:
            single_patch_feats = pusedo_kernels[begin_i:(begin_i+end_i)] \
                .repeat_interleave(20 // end_i, dim=0)
            single_patch_labels = kernel_labels[begin_i:(begin_i+end_i)] \
                .repeat_interleave(20 // end_i, dim=0) ######
            
            if 20 % end_i != 0:
                single_patch_feats = torch.cat((single_patch_feats, \
                    pusedo_kernels[begin_i:(begin_i+20 % end_i)]), dim=0)
                single_patch_labels = torch.cat((single_patch_labels, \
                    kernel_labels[begin_i:(begin_i+20 % end_i)]), dim=0) ######
            # 将kernels扩充到20个
            expend_kernels.append(single_patch_feats)
            expend_labels.append(single_patch_labels) ######
            begin_i += end_i
            
        expend_kernels = torch.cat(expend_kernels, dim=0)
        expend_labels = torch.cat(expend_labels, dim=0) ######


        return (expend_kernels.detach(), expend_labels.detach()) ######
