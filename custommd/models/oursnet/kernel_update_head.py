...
class KernelUpdateHead(nn.Module):
    ...
    def forward(self,
        ...
        cls_feat = obj_feat.sum(-2)  # used to generate cls_score
        mask_feat = obj_feat  # used to generate mask 
    ...
    def loss(self,
        ...
        if gt_kernels is not None:
            pred_kernels = []
            for patch_object_feat, gt_ins in zip(object_feats, gt_ins_id):
                pred_kernel = patch_object_feat[gt_ins, ...]
                pred_kernels.append(pred_kernel)
            pred_kernels = torch.cat(pred_kernels, dim=0).squeeze(-1).squeeze(-1)
            gt_kernels = torch.cat(gt_kernels, dim=0)

            flatten_object_feats = (pred_kernels / pred_kernels.norm(dim=1, keepdim=True)).squeeze(-1).squeeze(-1)
            flatten_gt_kernels = gt_kernels / gt_kernels.norm(dim=1, keepdim=True) 
            loss_emb = 1 - (flatten_object_feats * flatten_gt_kernels).sum(dim=-1)
            losses['loss_emb'] = loss_emb.mean() * 1.0
        ...
        if mask_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_avg'] = self.loss_mask(pos_mask_pred,
                                                     pos_mask_targets)
                losses['loss_projection'] = self.loss_dice(pos_mask_pred,  # 选择的实例
                                                     pos_mask_targets) # 真值
                ...
                losses['loss_avg'] = mask_pred.sum() * 0
                losses['loss_projection'] = mask_pred.sum() * 0
                if self.loss_rank is not None:
    ...
    def get_targets(self,
        ...
        return labels, label_weights, mask_targets, mask_weights, pos_inds_list
    ...
    def segm2result(self, mask_preds, det_labels, cls_scores):
        ...
        boxes = mask2bbox(mask_preds.bool())
